...
#' @param imu file name for imu_gps file, including extension .txt
#' @keywords 
#' @export
#' @examples
#' imu_framecombine()

imu_framecombine <- function(folderloc=paste0(RemoteSenDataLoc,FolderLoc),imu="imu_gps.txt"){
  framelist <- list.files(folderloc,pattern = "frameIndex")
  framematch <- rbindlist(lapply(framelist,function(x){fread(paste0(folderloc,x))}))

#coords.epsg = "32615" is utm 15n which is what I want here. "4326" is lat lon

imu <- read.delim(paste0(folderloc,"imu_gps.txt"))
